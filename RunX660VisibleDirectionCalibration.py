#!/usr/bin/env python3
"""Stage 4C visible X6-60 direction-calibration harness.

The default path is a dry run and does not open SocketCAN.  The armed path
performs one absolute raw-angle move of exactly +5.00 degrees at 1 degree/s.
It is deliberately independent of VanguardxMain_scheduler.py and does not
alter motor configuration, encoder zero, CAN settings, or scheduler settings.
"""
...
CALIBRATION_DELTA_DEG = 5.00
CALIBRATION_SPEED_DEG_PER_SEC = 1
MAX_TRAVEL_FROM_START_DEG = 5.50
MAX_ABS_CURRENT_A = 2.00
MAX_TEMPERATURE_C = 50
MAX_REPORTED_SPEED_DEG_PER_SEC = 3
TARGET_TOLERANCE_DEG = 0.15
REVERSE_MOTION_GUARD_DEG = 0.20
SETTLE_SAMPLES = 3
POLL_PERIOD_SEC = 0.05
MOVE_TIMEOUT_SEC = 12.0


def ParseArguments(Argv=None):
    Parser = argparse.ArgumentParser(
        description="Five-degree Stage 4C X6-60 visible direction calibration"
    )
    Parser.add_argument("--interface", default="can0")
    Parser.add_argument("--node-id", type=int, default=1)
    Parser.add_argument("--timeout-ms", type=float, default=250.0)
    Parser.add_argument("--execute-five-degree-calibration", action="store_true")
    Parser.add_argument(
        "--i-confirm-x6-60-is-secured-and-area-clear", action="store_true"
    )
    Parser.add_argument(
        "--i-confirm-at-least-six-degrees-free-rotation", action="store_true"
    )
    Parser.add_argument(
...
def ValidateArguments(Arguments):
    ...
    if Arguments.execute_five_degree_calibration:
        Required = (
            (
            ...
            (
                Arguments.i_confirm_at_least_six_degrees_free_rotation,
...
def DescribePlan(Arguments):
    print("Vanguard X Stage 4C X6-60 visible direction calibration")
    ...
    print(f"  speed guard:      {MAX_REPORTED_SPEED_DEG_PER_SEC} deg/s")
...
def ReadState(Bus, CanModule, Protocol, NodeId, TimeoutSec):
    Status1 = Protocol.DecodeStatus1(
        TransactRead(Bus, CanModule, Protocol, NodeId, Protocol.READ_STATUS_1, TimeoutSec)
    )
    ...
    return Status1, float(RawAngle), Status2


def CalculateTargetRawAngle(StartRawAngle):
    return float(StartRawAngle) + CALIBRATION_DELTA_DEG


def BuildCalibrationFrame(StartRawAngle):
    return BuildAbsolutePositionRequest(
        CalculateTargetRawAngle(StartRawAngle), CALIBRATION_SPEED_DEG_PER_SEC
    )
...
def ExecuteCalibration(Arguments):
    ...
    try:
        ...
        StartStatus1, StartRawAngle, StartStatus2 = ReadState(
        ...
        print(
            ...
            f"temp={StartStatus1.TemperatureC} C, "
            f"errors=0x{StartStatus1.ErrorFlags:04X}"
        )

        if StartStatus1.ErrorFlags:
            raise RuntimeError("preflight failed: motor reports error flags")
        if StartStatus1.TemperatureC > MAX_TEMPERATURE_C:
        ...
        TargetRawAngle = CalculateTargetRawAngle(StartRawAngle)
        Frame = BuildCalibrationFrame(StartRawAngle)
        print(f"  target raw angle: {TargetRawAngle:+.2f} deg")
        print("  COMMAND:          sending one 0xA4 absolute-position frame")
        TransactMotion(Bus, can, Protocol, Arguments.node_id, Frame, TimeoutSec)
        ...
        while time.monotonic() < Deadline:
            Status1, RawAngle, Status2 = ReadState(
                Bus, can, Protocol, Arguments.node_id, TimeoutSec
            )
            SampleIndex += 1
            ...
            if Status1.ErrorFlags:
                raise RuntimeError("motor reported error flags during movement")
            if Status1.TemperatureC > MAX_TEMPERATURE_C:
                raise RuntimeError("temperature guard exceeded")
            ...
            if abs(float(Status2.SpeedDegPerSec)) > MAX_REPORTED_SPEED_DEG_PER_SEC:
                raise RuntimeError("speed guard exceeded")
        ...
        if not ReachedTarget:
            raise TimeoutError("five-degree target did not settle before timeout")

        FinalStatus1, FinalRawAngle, FinalStatus2 = ReadState(
            Bus, can, Protocol, Arguments.node_id, TimeoutSec
        )
        ...
        print(
            ...
            "for the five-degree calibration command"
        )
        print(
        ...
        print(
            ...
            f"temp={FinalStatus1.TemperatureC} C, "
            f"errors=0x{FinalStatus1.ErrorFlags:04X}"
        )
    except Exception:
        if MotionCommandSent and not ReachedTarget:
            AttemptEmergencyStop(Bus, can, Protocol, Arguments.node_id, TimeoutSec)
        raise
    finally:
        Bus.shutdown()
...
def Main(Argv=None):
    ...
    if not Arguments.execute_five_degree_calibration:
